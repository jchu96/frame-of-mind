...
| Staged media | `sensitive-runtime` | Per-user application-data staging outside the checkout; hosted ephemeral bytes travel browser → Gemini and never enter Worker/D1 | Local: selected expiry and maintenance. Hosted: terminal/cancel/janitor deletion plus provider expiry | Current local operator or hosted principal, authorized runtime, and Gemini | [local media implementation](../apps/web/server-local/studio-media/local-media-staging.ts), [hosted contract](../scripts/test-hosted-media-http.ts) |
| Hosted retained recordings and captured frames | `sensitive-runtime` | Private principal-owned R2 objects; D1 holds opaque keys, lifecycle receipts, capture digests, and manifest/recording/timestamp provenance but no media bytes | Visible retained-until policy (30 days by default), earlier explicit owner delete, application janitor, and bucket lifecycle backstop; incomplete multipart uploads abort within seven days | Owning Better Auth principal and authorized Cloudflare runtime/operators | [ADR 0018](adr/0018-hosted-studio-trust-boundary.md), [retention/evidence contract](../scripts/test-hosted-media-http.ts) |
...
| API keys, OAuth tokens, encryption keys, and signed URLs | `sensitive-runtime` | Process environment or ignored `.env`, process memory, exact-resource private OAuth files, provider/browser memory, and Cloudflare secret storage for hosted deployments | Until process exit, explicit disconnect/revocation, URL expiry, or operator rotation | Credential owner and the exact authorized runtime/provider boundary | [credentials guide](CREDENTIALS.md), [ADR 0008](adr/0008-local-secret-resolution.md), [OAuth isolation tests](../test/oauth.test.ts) |
| Hosted identities (`ba:<userId>`, compatibility Access `sub`, display email) | `internal` | Validated request context and principal-scoped D1 ownership columns; email is admission/display data only | Auth session plus the owning D1 retention period | The authenticated principal and authorized deployment operators | [auth middleware tests](../apps/web/test/access.test.ts), [principal migration](../apps/web/db/migrations/0003_principal_scope.sql) |
...
| D1 rows | `sensitive-runtime` | Principal-scoped hosted run projections and operational tables | Workspace policy and exact-principal/exact-run purge; current automation does not expire completed projections | The owning Better Auth principal and authorized Cloudflare operators | [D1 ownership tests](../apps/web/test/d1.test.ts), [deployment backup/purge](CLOUDFLARE_DEPLOYMENT.md#database-backup) |

Hosted recording upload is live on the reference instance as of 2026-08-23.
ADR 0018 Amendment 2 uses a short-lived browser-to-Gemini
capability: the public Worker holds the key, D1 holds only principal/media-bound
ciphertext plus declarations, and the browser holds the plaintext URL only in
one tab's session draft. Private R2 retained media and evidence capture use the
visible lifecycle above. See the
