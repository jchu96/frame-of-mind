...
The fake GitHub authorization/token/user server and HTTP fallback mailer
contain no production client secret. Playwright performed both sign-ins against
the built Worker. The binding variant used Wrangler's local `send_email`
simulator with a restricted invited recipient, then inspected the isolated
plain-text capture for the link and expiry copy; no test binding used
`remote: true`. Unknown email received only the sanitized
